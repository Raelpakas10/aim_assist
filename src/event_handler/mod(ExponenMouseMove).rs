...
fn update_analog(&mut self) {
    ...
    let Sensitivity = if self.parachute_Sensitivity_active {
        self.config.Parachute_Sensitivity
    } else {
    ...
    if self.parachute_Sensitivity_active {
        ...
        self.set_analog_exponential(analog_x, analog_y);
    }
}
    ...
    fn set_analog_exponential(&mut self, x: f64, y: f64) {
        let magnitude = (x.powi(2) + y.powi(2)).sqrt();
        let angle = y.atan2(x);

        self.report.s_thumb_rx = (angle.cos() * magnitude * Self::ANALOG_MAX) as i16;
        self.report.s_thumb_ry = (angle.sin() * magnitude * Self::ANALOG_MAX) as i16;
    }
}